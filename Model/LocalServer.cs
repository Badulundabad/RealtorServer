...
{
    public class LocalServer : INotifyPropertyChanged
    {
        Boolean isRunning;
        ...
        public ObservableCollection<LocalClient> OnlineClients { get; private set; }
        #endregion
        //The Server should add the address from message came
        public LocalServer(Dispatcher dispatcher)
        {
            uiDispatcher = dispatcher;
            OperationResults = new List<Operation>();
            Log = new ObservableCollection<LogMessage>();
            OnlineClients = new ObservableCollection<LocalClient>();
                    ...
                    using (listeningSocket = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp))
                    {
                        ...
                        do
                        {
                            if (listeningSocket.Poll(100000, SelectMode.SelectRead))
                            {
                                ...
                                LocalClient client = new LocalClient(clientSocket, this);
                                client.PropertyChanged += (sender, e) =>
                                {
                                    if (e.PropertyName == "IsConnected")
                                    {
                                        if (!((LocalClient)sender).IsConnected)
                                            uiDispatcher.BeginInvoke(new Action(() =>
                                            {
                                                OnlineClients.Remove((LocalClient)sender);
                                            }));
                                    }
                                };
        ...
        public void Stop()
        {
            try
            {
                ...
                if (OnlineClients.Count > 0)
                    foreach (LocalClient client in OnlineClients)
                    {
                        client.Disconnect();
                    }
            }
        ...
        private void OnProperyChanged([CallerMemberName] String prop = null)
        {
